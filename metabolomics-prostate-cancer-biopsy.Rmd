...
data.2 <- select(data, -(treatment_type:cancer))
data.2
...
The majority of samples (~70%) are benign, 17% of samples have a Gleason grade of 3+3 and are considered "low grade" tumors, and the other ~13% of samples are considered malignant or "high grade" tumors. From the boxplot, there are two groups labeled "G4+3", indicating an issue with labeling in the initial datatset. We will use `str_trim` to remove any whitespace in `gleason_grade`. 
```{r}
# remove whitespace
...
# Use k with lowest error; k = 1 and 5 had lowest misclassification error
final.k = 5
```
...
Here, `confusionMatrix` indicates that the accuracy of our model was 78%. Sensitivity was 0.47 and specificity was 0.89.
```{r}
################################################################################
